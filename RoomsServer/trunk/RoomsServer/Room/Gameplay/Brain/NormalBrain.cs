...
namespace RoomsServer
{
	public class NormalBrain : Brain
	{
		...
		public override FiledCursor Step(FiledCursor opponentSetep)
		{
		...
		private FiledCursor OneLeft(Point position)
		{
			FiledCursor rv = null;

			var sides = Sides(position);
			if (sides.Count(s => s) == 1)
				rv = new FiledCursor() { X = position.X, Y = position.Y, Direction = (Direction)Array.IndexOf(sides, true) };
			return rv;
		}

		private int HowManyTake(FiledCursor cursor)
		{
			FiledCursor c = (FiledCursor)cursor.Clone();
			FiledCursor c1 = c.Mirror;
		...
		private bool Free(FiledCursor cursor)
		{
